...
import { addLove, retrieveLove } from "src/store/slices/itemsSlice";
...
function Item() {
    ...
    async function addToWish() {
        if (userConnected) {
            ...
            if (userWish.find((item) => parseInt(item) == router.query.id)) {
                ...
                await updateDoc(itemRefrence, {
                    wishlist: userWish.filter((item) => parseInt(item) != router.query.id),
                });
    ...
    const diapoImages = [item.mainPic, item.secPic, item.thirdPic];
        ...
        <div className="md:min-h-screen text-black w-full pt-20 md:flex justify-center seconde:px-40 md:pb-10">
            <div className="flex flex-col w-full px-3 pt-5 gap-1 md:flex-5">
                ...
                <div className="flex gap-1">
                    <div
                        className={imageSelected == 0 && "opacity-50 scale-[0.95] transition-all"}
                        onClick={() => setImageSelected(0)}>
                        <img src={diapoImages[0]} alt="" />
                    </div>
                    <div
                        className={imageSelected == 1 && "opacity-50 scale-[0.95] transition-all"}
                        onClick={() => setImageSelected(1)}>
                        <img src={diapoImages[1]} alt="" />
                    </div>
                    <div
                        className={imageSelected == 2 && "opacity-50 scale-[0.95] transition-all"}
            ...
            <div className=" md:flex-7 md:pr-40 md:pl-10 md:pt-10">
                ...
                <div className="display flex  justify-around items-center">
                    ...
                    <div className="flex flex-col seconde:flex-row justify-center items-center border seconde:border-none seconde:flex-auto rounded-md px-5 py-3 text-lg text-gray-400">
                        <img className="w-1/3 seconde:w-10 mb-2 seconde:mr-5" src="/icons/sun-icon-black.svg" alt="" />
                        <p>Needs sun</p>
                    </div>
                </div>
                <div className="mt-4 ml-4 flex flex-col">
                    <InfoDrop titleInfo="Description" descriptionInfo={item.description} descHeight={200} />
                    <InfoDrop titleInfo={`how to treat a ${item.type}`} descriptionInfo={item.likes} descHeight={200} />
                </div>

                <div className="flex gap-5 mt-16 items-center md:float-right">
                    <button
                        onClick={addToCart}
                        className="flex-auto md:flex-initial md:px-10 md:rounded-md transition  bg-[#7D916C] hover:bg-[#6e805e] text-white border border-[#7D916C] py-5 text-[18px]">
                        Add to cart
                    </button>
                    <div
                        onClick={addToWish}
                        className=" rounded-full p-2 transition hover:bg-[#7c7c7c2c] cursor-pointer text-center">
                        {userConnected && userWish.find((item) => item == router.query.id) ? (
                            <img className="w-10" src="/icons/heart-icon-black-filled.svg" alt="" />
                        ) : (
