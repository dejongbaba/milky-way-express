...
function TopSellingItem({ title, subTitle, price, src, description = "...", id }) {
    return (
        <motion.div whileHover={{ scale: 1.02 }} className="text-black p-5 md:w-[350px] w-[200px] h-full flex flex-col">
            ...
            <button className="transition hover:bg-[#657457] hover:text-white border-[#7D916C] border text-[#7D916C] text-white py-3 px-6 text-[18px] yoyo">
                <Link href={{ pathname: "/products/[id]", query: { id: id } }}>{`Buy for ${price}`}</Link>
            </button>
        </motion.div>
    );
}
