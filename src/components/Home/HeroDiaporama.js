...
function HeroDiaporama() {
    ...
    return (
        <div className="relative w-[600px] h-[600px] bg-white rounded-full z-20 mr-50 seconde:block hidden">
            <motion.button className="cursor-pointer w-24 absolute right-0 top-[260px] z-30">
                <img className="w-full" src="icons/arrow-icon.svg" alt="" onClick={nextButton} />
            </motion.button>
            <motion.button className="cursor-pointer w-24 absolute left-0 top-[260px] rotate-180 z-30">
                <img className="w-full" src="icons/arrow-icon.svg" alt="" onClick={previousButton} />
            </motion.button>
            ...
            {plants.map((item, index) => {
                return (
                    <AnimatePresence key={item.title}>
                        ...
                        {current === item.id && (
                            <motion.img
                                initial={{ opacity: 0, x: -120, y: 0, scale: 0.5 }}
                                animate={{ opacity: 1, x: 0, y: 0, scale: 1 }}
                                exit={{ opacity: 0, x: 100, y: 0, scale: 0.5 }}
            ...
            <div className="absolute right-28 top-[210px] text-black">
                ...
                <motion.p
                    ...
                    className="text-2xl text-[#737373] mb-12">
                    {plants[current].price}
                </motion.p>

                <Link
                    href={{ pathname: "/products/[id]", query: { id: current == 0 ? "1" : current == 1 ? "2" : "3" } }}>
                    <button className="transition hover:bg-[#657457] bg-[#7D916C] text-white py-3 px-6 rounded-full text-[20px]">
                        view details
                    </button>
                </Link>
            </div>
