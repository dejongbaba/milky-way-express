        ...
        <div className="min-h-screen w-full text-black flex justify-around items-center">
            <div className="px-14 py-8 text-xl">
                ...
                <Link href={{ pathname: "/products" }}>
                    <button className="border-[0.7px] border-[#7D916C] py-2 px-4 text-[#7D916C] hover:bg-[#7D916C] hover:text-white transition">
                        Back to Plants
                    </button>
                </Link>
            </div>
                        ...
                        {cartItems.map((item) => {
                            return (
                                <div key={item.id} className="flex justify-between">
                                    <p>
                                        <span className="font-normal">{item.title}</span> x <span>{item.quantity}</span>
                                    </p>
